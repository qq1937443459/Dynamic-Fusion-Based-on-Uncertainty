...
class ChannelWeights(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        B_, C_, T_, V_ = x1.shape
        avg1 = self.avg_pool(x1).view(B_, C_)
        avg2 = self.avg_pool(x1).view(B_, C_)
        avg = torch.cat((avg1, avg2), dim=1)
        max1 = self.max_pool(x2).view(B_, C_)
        max2 = self.max_pool(x2).view(B_, C_)
        max = torch.cat((max1, max2), dim=1)

        y = torch.cat((avg, max), dim=1)  # B 2C
        y = self.mlp(y).view(B_, C_ * 2, 1)
        channel_weights = y.reshape(B_, 2, self.dim, 1, 1).permute(1, 0, 2, 3, 4) # 2 B C 1 1
...
class FRM(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        out_x1 = x1 + self.lambda_c * channel_weights[1] * x1
        out_x2 = x2 + self.lambda_c * channel_weights[0] * x2
...
if __name__ == '__main__':
    ...
    input1 = torch.randn(8, 512, 16, 16)
