...
from models.test2_model import Module
...
def load():
    ...
    y_pre = []
    uncertainty_ = []
    # train_data = CRPTorchDataset('train')
    # train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=4)
    # test_data = CRPTorchDataset('test')
    ...
    model = fusion_model()
    model.to(device)
    # model.load_state_dict(torch.load('./checkpoints/epoch_79_2024-06-01-20-46-07.pkl'), strict=False)  # mse

    # model.load_state_dict(torch.load('./checkpoints/epoch_8_fusion_2024-06-15-16-08-41.pkl', map_location=device), strict=False)   # edl
    # model.load_state_dict(torch.load('./checkpoints/epoch_4_fusion_2024-06-15-15-12-55.pkl', map_location=device), strict=False)   #
    model.load_state_dict(torch.load('./checkpoints2/epoch_15_fusion_mse_2024-06-23-15-59-47.pkl', map_location=device), strict=False)   #
    lu = []
    errors = []
    ...
    for _, samples in loop:
        ...
        loss, feat_r, feat_v, feat_f = model(radar_feature, new_visual, targets, 1)
        ...
        uncertainty = 10 * 1.8 / torch.sum(1.8 ** alpha_r, dim=1, keepdim=True)
    ...
    draw_matrix(label_ture=y, average_unc=average_unc,
                ...
                title="confusion Maatrix", pdf_save_path="matrix/mse23/out_test_r.jpg", dpi=300)
